...
import androidx.core.app.ActivityCompat;import android.text.TextUtils;
...
public class ScanAndWinPromocodeDialog extends Dialog {
    private ImageView imgBag;
    private int rotationCount = 0;
    private TextView tvNo, tvYes;
    public static final int SCANNER_REQUEST_CODE = 0x11;
    private static ScanAndWinPromocodeDialog f;
    ...
    public static final int CAMERA_PERMISSION_REQUEST = 0x31;
    final String[] permsCamera = {"android.permission.CAMERA"};

    public ScanAndWinPromocodeDialog(Context context, String path) {
        super(context);
        this.mContext = context;
        this.path = path;
    }

    public ScanAndWinPromocodeDialog(Context context, int themeResId, String path, boolean isForScan) {
    ...
    protected ScanAndWinPromocodeDialog(Context context, boolean cancelable, OnCancelListener cancelListener, String path) {
        super(context, cancelable, cancelListener);
        this.mContext = context;
        this.path = path;
    }

    public static ScanAndWinPromocodeDialog newInstance(Context mContext, String imgPath, boolean isForScan) {
        if (f == null) {
            int theme = R.style.LoadingDialog;
            f = new ScanAndWinPromocodeDialog(mContext, theme/*android.R.style.Theme_Translucent_NoTitleBar_Fullscreen*/, imgPath, isForScan);
        }
        return f;
    }
...
//                        ((AddPromocodeActivity) mContext).setScanImage();
//                    }
                } else if (isForScan) {
                ...
                ScanAndWinPromocodeDialog.this.dismiss();
            }
        });
...
//        }
    ...
    public void dismiss() {
        ...
        editor.putBoolean(AppConstants.USER_SCAN_RESULT, true);
        editor.apply();
//        if (mContext instanceof AddPromocodeActivity) {
//            ((AddPromocodeActivity) mContext).setScanImage();
//        }
        super.dismiss();
        System.gc();
